Metabolism = function(nDays,nYears2Save,myLake,myParameters,Irradiance,
  ...
  for (i in 2:nDays){
    ...
    RatesEpiP$TPSettling[i] = min(StatesEpiP$TP[i-1],StatesEpiP$TP[i-1] * TPcSed * 
      TPSedimentationTheta^(LakeTemps$EpiT[i]-TPSedimentationBaseT) / MeanEpiThickness)
    # Epi gas exchange
    ...
    if (StatesEpiGas$DO[i-1]/LakeVolumes$EpiVol[i-1] <= AnoxiaThreshold ){ # Anoxic
      # Release
      RatesEpiP$TPRelease[i] = PropSedInEpi * StatesSedP$TPB[i-1] * TPcRelease #recycling in g/day
      ...
      RebindEpi = PropSedInEpi * (StatesEpiP$TP[i-1] * TPcRebind) # Potential rebinding
    ...
    if (isStratified & DayOfStratification > 1){ # Stratified, note ">" for begin, which allows indexing to work
      ...
      if (StatesHypoGas$DO[i-1]/LakeVolumes$HypoVol[i-1] <= AnoxiaThreshold ){ # Anoxic
        # Release
        RatesHypoP$TPRelease[i] = PropSedInHypo * StatesSedP$TPB[i-1] * TPcRelease #recycling in g/day
        RatesHypoP$TPRebind[i] = 0
      }else{ # Oxic
        # Rebind if room available in bound P pool
        # Calculate static seds available for binding
        AvailSedBoundHypo = max(0,PropSedInHypo * (SedTPBoundMaxgrams - StatesSedP$TPB[i-1]))  # in grams
        RebindHypo = PropSedInHypo * (StatesHypoP$TP[i-1] * TPcRebind) # Potential rebinding
