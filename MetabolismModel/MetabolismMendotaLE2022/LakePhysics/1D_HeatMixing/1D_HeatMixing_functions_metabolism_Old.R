...
get_interp_drivers <- function(meteo_all, total_runtime, hydrodynamic_timestep, dt){
  times = seq(1, 1 + total_runtime*hydrodynamic_timestep, dt)
  meteo <- matrix(NA, ncol = total_runtime * hydrodynamic_timestep/dt + 1,
                  nrow = 9)
...
run_thermalmodel <- function(u, startTime, endTime, 
  ...
  if (pgdl_mode == 'on'){
    um_diff <- matrix(NA, ncol =N_steps , nrow = nx)
    um_mix <- matrix(NA, ncol =N_steps , nrow = nx)
    um_conv <- matrix(NA, ncol =N_steps , nrow = nx)
    um_ice <- matrix(NA, ncol =N_steps , nrow = nx)
    n2_pgdl <- matrix(NA, ncol = N_steps, nrow = nx)
    
    meteo_pgdl <- matrix(NA, ncol = N_steps, nrow = 9)
  }
  ...
  for (n in 1:N_steps){#1:(floor(endTime/dt - startTime/dt))){  #iterate through time 1:floor(nt/dt)
    ...
    ## (1) Heat addition
    # surface heat flux
    Q <- (absorp * daily_meteo[1,n] + longwave(cc = daily_meteo[6,n], sigma = sigma, Tair = daily_meteo[3,n], ea = daily_meteo[4,n], emissivity = emissivity, Jlw = daily_meteo[2,n]) + #longwave(emissivity = emissivity, Jlw = daily_meteo[2,n]) +
    ...
    if (scheme == 'explicit'){
      ...
      # }
      u[2:(nx-1)] = un[2:(nx-1)] +
            (area[2:(nx-1)] * kzn[2:(nx-1)] * 1 / dx**2 * (un[3:nx] - 2 * un[2:(nx-1)] + un[1:(nx-2)]) +
               abs(H[3:(nx)]-H[2:(nx-1)]) * area[2:(nx-1)]/(dx) * 1/(4184 * calc_dens(un[2:(nx-1)]) ) +
               Hg[2:(nx-1)])* dt/area[2:(nx-1)]
      # bottom layer
      u[nx] = un[nx] +
    ...
    for (dep in 1:(nx-1)){
      if (dep == 1){
        ...
        # PE = abs(g *  ( seq(1,nx)[dep] - Zcv)  * calc_dens(u[dep]) * dx +
        #            PEprior)
    PE = abs(g *   seq(1,nx)[dep] *( depth[dep+1] - Zcv)  *
    ...
    # PE_depth = abs(g * seq(1,nx)[1:(nx-1)] *( seq(1,nx)[2:nx] - Zcv)  *
    #                           abs(calc_dens(u[1:(nx-1)])- cumsum(calc_dens(u[1:(nx-1)])) / (1:(length(u)-1))))
    # PE = cumsum(PE_depth)
    # mix_depth = max(which(PE < KE))
    # if(is.infinite(mix_depth)){
    #   maxdep = 0
    #   mix[n] <- KE/PE[1]
    # }else{
    #   maxdep = mix_depth
    #   u[1:maxdep] = (u[1:maxdep] %*% volume[1:maxdep])/sum(volume[1:maxdep])
    #   mix[n] <- KE/PE[maxdep]
    # }
    # u[1:maxdep] = (u[1:(maxdep)] %*% volume[1:(maxdep)])/sum(volume[1:(maxdep)]) #mean(u[1:maxdep])
    mix[n] <- KE/PE #append(mix, KE/PE) # KE/PE[maxdep]
  ...
  colnames(df.sim) <- c("datetime", as.character(paste0('wtemp.',depth)))
  df.sim$datetime <-   seq(startTime, endTime, dt)#/24/3600
  
  ## averaged responses
  bf.sim <- apply(df.sim[,-1], 1, function(x) rLakeAnalyzer::buoyancy.freq(wtr = x, depths = depth))
  
  bf.sim <- apply(df.sim[,-1], 1, function(x) rLakeAnalyzer::center.buoyancy(wtr = x, depths = depth))
  ...
  avg.hyp.sim <- NULL
  avg.tot.sim <- NULL
  for (j in 1:nrow(df.z.df.sim)){ # TODO: can this be vectorized???
    d = df.sim[,-1]
  ...
  stratFlag = rep(NA, length = N_steps)
