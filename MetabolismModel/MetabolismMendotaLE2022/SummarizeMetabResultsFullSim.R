SummarizeMetabResultsFullSim = function(MetabResults,myLake,LakeHypsometry,nYears2Save,Print){
  print('so far so good')
  AnoxicThreshold = 1 # Below this value, we consider the system anoxic
  # Setup indeces for date ranges
  nRecs2Save = length(MetabResults[[1]]$SimDay)-1
  FirstDay=2 # This gets changed if there's only on year because of NAs
  ...
  MassBalanceP$dPsed = ((StatesSedP$TP[nRecs2Save]+ StatesSedP$TPB[nRecs2Save]) - 
                          (StatesSedP$TP[FirstDay]+StatesSedP$TPB[FirstDay])) / LakeArea
