...
# Lake
desis_binned <- read.csv("Data/DESIS/vnorm_desis_lake_500clip_binnedAfterNormalize.csv")
#desis_buoy <- read.csv("Data/DESIS/L2W_vnorm/vnorm_desis_buoy.csv")

# Buoy
dsby_binned <- read.csv("Data/DESIS/vnorm_desis_buoy_500clip_binnedAfterNormalize.csv")

# pivot to long
des_bin_lon <- desis_binned %>% 
  rownames_to_column("index") %>%
  pivot_longer(cols = 5:117,
             names_to = "wavelength",
             values_to = "values",
             names_prefix = "Rrs_") %>%
  mutate(wavelength = as.numeric(wavelength)) 

# Check data first - does not work w/binned data --------------------------
desis_mean <- des_bin_lon %>% # not working
  group_by(as.factor(date), as.factor(wavelength)) %>%
...
library(zoo)
desis_smooth <- des_bin_lon %>%
  group_by(index, date) %>%
  arrange(wavelength) %>%
  mutate(mov_avg = rollmean(values,
                            k = 2,
                            fill = NA,
                            align = "left"))
...
desis_smooth_wide <- pivot_wider(desis_smooth,
                                 ...
                                 values_from = values) #? should be mov_avg?
...
## LAKE ----------
desis_sub = desis_binned %>% #come back to if we do need to smooth
  group_by(date) %>% 
  drop_na() %>% 
  slice_sample(prop = 0.10) 

desis_pca_input <- desis_sub %>% ungroup() %>% select(-c(date, x, y)) #index

pca_dss <- princomp(desis_pca_input, cor = TRUE)

## BUOY -----------

desis_buoy_pca_input <- dsby_binned %>% ungroup() %>% 
  dplyr::select(-c(date,x,y))

pca_buoy_dss <- princomp(desis_buoy_pca_input,cor=TRUE)
pca_buoy_dss_scores = as.data.frame(pca_buoy_dss$scores)
pca_buoy_scores_mean <- cbind(dsby_binned %>% dplyr::select(date), pca_buoy_dss_scores) %>%
  group_by(date) %>%
  summarize_all(.funs = mean)

saveRDS(pca_buoy_scores_mean, "Outputs/PCA_outputs/DESIS/desis_buoy_pca_means_1024.rds")

# PCA plotting & output export -----------
...
write_csv(desis_sub[,c(1,3:5)],"Outputs/PCA_outputs/DESIS/desis_geo.csv" )

screeplot(pca_dss)#shows variance represented by each component
#improve biplot
library(ggfortify)
pds <- autoplot(pca_dss,loadings = TRUE,loadings.label = T,
                loadings.label.repel = T, loadings.colour = "black",
...
dss_gg <- ggord(pca_dss, desis_sub$date,
                cols=c("firebrick4","orange", "yellow2",
                       "yellowgreen","cornflowerblue",
                       "purple3","pink3","deeppink2"), ellipse = FALSE, vec_ext = 4.5,
                vectyp="solid",xlims=c(-4,3),ylims=c(-3,4),
                repel=TRUE,grp_title="Bout",size=3,max.overlaps= 100)
...
# #try top ten for now
vari10 <- variances %>% filter(component %in% c("Comp.1","Comp.2","Comp.3","Comp.4","Comp.5","Comp.6", "Comp.7","Comp.8","Comp.9","Comp.10"))
...
write_csv(loadings, "Outputs/PCA_outputs/DESIS/desis_PCAtop10_loadings.csv")
write_csv(vari10, "Outputs/PCA_outputs/DESIS/desis_PCAtop10_variances.csv")
