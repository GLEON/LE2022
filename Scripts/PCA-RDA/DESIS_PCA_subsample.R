...
dss_gg<-ggord(pca_dss,desis_subW$date,cols=c("firebrick3","orange","yellowgreen","cyan3","cornflowerblue","purple3","pink3","deeppink4"),ellipse=FALSE,vec_ext=4.5,vectyp="solid",xlims=c(-4,3),ylims=c(-3,4),repel=TRUE,grp_title="Bout",size=3,max.overlaps=50)
...
loadings = pca_dss$loadings[,1:nrow(variances)] %>% #only keep the ones that have been retained
...
write_csv(loadings, "Data/DESIS/desis_PCAtop_loadings.csv")
write_csv(variances, "Data/DESIS/desis_PCA_variances.csv")
