...
DESIS_dat = read.csv('Data/DESIS/DESIS_stats_buoy.csv') %>% 
  select(-X) %>% 
  pivot_longer(!date, names_to = c('wavelength', 'variable'), names_sep = '_', names_prefix = 'X') %>% 
...
ggplot(filter(DESIS_dat, avg > -1))+
  theme_bw()+
  geom_line(aes(x = wavelength, y = avg))+
  geom_line(aes(x = wavelength, y = avg + sd),color = "blue", alpha = 0.25)+
  geom_line(aes(x = wavelength, y = avg - sd),color = "blue", alpha = 0.25)+
  facet_wrap(~date, scales = 'free_y')+
  labs(x = "Wavelength (nm)", y = "Vector-normalized Rrs", 
       title = 'DESIS spectra from buoy area')
...
DESIS_lake <- read_csv('Data/DESIS/DESIS_stats_lake.csv') %>% 
  select(-`...1`) %>% 
  pivot_longer(cols = -date, names_to = c('wavelength', 'variable'), names_sep = '_', names_prefix = 'X') %>% 
...
ggplot(DESIS_lake)+
  ...
  geom_line(aes(x = wavelength, y = avg - sd),color = "blue", alpha = 0.25)+
  facet_wrap(~date, scales = "free_y")+
  labs(x = 'Wavelength (nm)', y = 'Vector-normalized Rrs', title = 'DESIS spectra from whole lake')
...
PRISMA_buoy <- read_csv('Data/PRISMA/PRISMA_stats_buoy.csv') %>% 
  select(-'...1') %>% 
  pivot_longer(cols = -date, names_to = c('wavelength', 'variable'), names_prefix = 'Rrs_', names_sep = '_') %>% 
  pivot_wider(names_from = variable) %>% 
  mutate(wavelength = as.numeric(wavelength))
...
ggplot(PRISMA_buoy)+
  ...
  lims(x=c(450,800))+
  labs(x='Wavelength (nm)', y = 'Vector-normalized Rrs',
       title = 'PRISMA spectra from buoy')
...
ggplot(PRISMA_lake)+
  ...
  lims(x=c(450,1000))+
  labs(x = 'Wavelength (nm)', y = 'Vector-normalized Rrs',
       title = 'PRISMA spectra from whole lake')
