...
buoy_corrected <- buoy %>%
  mutate(scaled_chlor_rfu_corr = scaled_chlor_rfu/(1+rho*(avg_do_wtemp-Tr)), # chlorophyll
         scaled_phyco_rfu_corr = scaled_phyco_rfu/(1+rho*(avg_do_wtemp-Tr)), # phycocyanin
...
# chlorophyll
p1 <- buoy_corrected %>%
  select(c(year, doy, sampledate, scaled_chlor_rfu, scaled_chlor_rfu_corr)) %>%
  pivot_longer(scaled_chlor_rfu:scaled_chlor_rfu_corr, names_to = "parameter") %>%
...
buoy %>% 
  ...
  filter(parameter %in% c("scaled_chlor_rfu", "scaled_phyco_rfu", "avg_fdom", "avg_turbidity")) %>%
...
buoy_clean <- buoy_clean %>% 
  select(c(year, sampledate, doy, scaled_chlor_rfu_corr, scaled_phyco_rfu_corr, avg_fdom_corr, avg_turbidity)) %>%
  subset(year %in% c(2019, 2020, 2021, 2022))
buoy_clean %>% 
  pivot_longer(scaled_chlor_rfu_corr:avg_turbidity, names_to = "parameter") %>% 
  filter(parameter %in% c("scaled_chlor_rfu_corr", "scaled_phyco_rfu_corr", "avg_fdom_corr", "avg_turbidity")) %>%
  ggplot(aes(x = sampledate, y = value)) +
  geom_point() +
  facet_wrap(~parameter, scales = "free_y", ncol = 1)
