...
####################################################
#s3
s3_buoy <- read.csv('Data/Sentinel3/s3_all_clip2buoy.csv') %>%
  dplyr::select(c(date, contains("rhos"))) %>%
  dplyr::select(-c(rhos_401,rhos_681)) %>%
  t() %>%
  row_to_names(row_number = 1)
s3_bands <- read.csv('Data/Sentinel3/s3_bands.csv')
s3_buoy <- cbind(s3_bands,s3_buoy)
s3_buoy_long <- s3_buoy %>%
  pivot_longer(!Wavelength_nm,names_to="date",values_to="rhos_s3")
s3_buoy_long$Wavelength_nm <- as.integer(s3_buoy_long$Wavelength_nm)
s3_buoy_long$rhos_s3 <- as.numeric(s3_buoy_long$rhos_s3)
s3_buoy_long_clean <- s3_buoy_long[!is.na(as.numeric(s3_buoy_long$rhos_s3)), ] 

##select out DESIS dates
s3d<-s3_buoy_long_clean%>%filter(date %in% DESIS_buoy_long_clean$date)
##
DESIS_buoy_long_clean  %>%
  ggplot(aes(x=Wavelength_nm,y=Rrs)) +
  geom_line() +
  geom_line(data=subset(s3d,!is.na(rhos_s3)),aes(x=Wavelength_nm,y=rhos_s3),color="darkcyan")+
  facet_wrap(~date) +
  theme_bw() +
  xlim(440,865) +
  #ylim(0,0.30) +
  labs(title='DESIS-S3 buoy-clipped mean spectra')

ggsave("Data/Sentinel3/desis_s3_buoy_avg_spectra.png")
################################################################################
#bring in S3 and then match w/PRISMA and DESIS
...
########################################################
##select out PRISMA dates
s3_buoy_long_clean$date<-gsub("[^[:alnum:]]", "_", tolower(s3_buoy_long_clean$date))

s3p<-s3_buoy_long_clean%>%filter(date %in% PRISMA_buoy_long_clean$date)

##
PRISMA_buoy_long_clean  %>%
  ggplot(aes(x=Wavelength_nm,y=Rrs)) +
  geom_line() +
  geom_line(data=subset(s3p,!is.na(rhos_s3)),aes(x=Wavelength_nm,y=rhos_s3),color="darkcyan")+
  facet_wrap(~date,scales = "free") +
  theme_bw() +
  xlim(440,1020) +
  #ylim(0,0.30) +
  labs(title='PRISMA-S3 buoy-clipped mean spectra')

ggsave("Data/Sentinel3/prisma_s3_buoy_avg_spectra.png")
